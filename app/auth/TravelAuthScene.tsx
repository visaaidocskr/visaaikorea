...
// No images, no canvas, no JS timers — the only script is a pointer handler
// that feeds two CSS variables for the 3D tilt/parallax, so the auth page
// stays fast even on a phone. The scene is hidden from assistive technology
// because the login form already supplies all meaningful information.
//
...
// Wind streaks trailing the traveler: x, y, delay.
const WIND: Array<[number, number, string]> = [
  [148, 238, "0s"],
  [136, 262, "-0.6s"],
  [158, 284, "-1.1s"],
];
...
export function TravelAuthScene() {
  ...
  // Mouse-driven tilt/parallax. Written straight to CSS variables (no state,
  // no re-renders); CSS decides how much each layer moves. Touch pointers and
  // reduced-motion users are left out on purpose.
  ...
  return (
    ...
    >
      <div className="auth-tilt absolute inset-0 px-6 py-7 md:px-10 md:py-10">
        ...
        <div className="auth-parallax auth-parallax-world absolute inset-0">
          ...
          >
            <defs>
              ...
              <linearGradient id="shoot-grad" x1="0" y1="0" x2="1" y2="0">
            ...
            <g className="auth-shooting-star" opacity="0">
              <path d="M0 0L34 -8" stroke="url(#shoot-grad)" strokeWidth="2" strokeLinecap="round" />
            </g>

            {/* The route ignites on load, then keeps breathing. */}
            <path
            ...
            {/* Wind streaks: the city stands still, the traveler moves. */}
            {WIND.map(([x, y, delay]) => (
              <path
                key={`${x}-${y}`}
                className="auth-wind"
        ...
        <div className="auth-parallax auth-parallax-text relative z-10">
          <div className="auth-enter flex items-center gap-2 text-xs font-bold uppercase tracking-[0.22em] text-blue-200">
            <span className="auth-beacon h-2 w-2 rounded-full bg-cyan-300 shadow-[0_0_14px_3px_rgba(103,232,249,0.55)]" />
            {t("auth.journey")}
          </div>
          <div className="auth-enter mt-3 max-w-xs" style={{ animationDelay: "120ms" }}>
