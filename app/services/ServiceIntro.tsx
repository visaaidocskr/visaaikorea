...
export function ServiceIntro({ kind }: { kind: "flight" | "tour" }) {
  ...
  return <section className="lg:pt-8">
    ...
    <p className="mt-8 inline-flex items-center gap-2 text-sm font-bold uppercase tracking-widest text-blue-600"><span aria-hidden className="sparkle text-cyan-500">✦</span>{t(isFlight ? "flight.eyebrow" : "tour.eyebrow")}</p>
    <h1 className="text-sky-gradient mt-3 text-4xl font-extrabold tracking-tight sm:text-5xl">{t(isFlight ? "flight.title" : "tour.title")}</h1>
