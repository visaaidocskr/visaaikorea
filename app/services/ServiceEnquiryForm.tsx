...
export function ServiceEnquiryForm({ kind }: { kind: ServiceKind }) {
  ...
  return (
    <form onSubmit={submit} className="space-y-7 rounded-3xl border border-slate-200 bg-white p-6 shadow-sm sm:p-8">
      <div>
        <h2 className="text-2xl font-extrabold text-slate-900">{t("form.request")} {label.toLowerCase()}</h2>
      ...
      <fieldset className="grid gap-4 sm:grid-cols-2">
        <legend className="mb-1 text-sm font-bold text-slate-900">{t("form.details")}</legend>
      ...
      <fieldset className="grid gap-4 sm:grid-cols-2">
        <legend className="mb-1 text-sm font-bold text-slate-900">{t("form.travel")}</legend>
