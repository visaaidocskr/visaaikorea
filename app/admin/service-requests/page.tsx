...
export default async function ServiceRequestsPage() {
  ...
  return <main className="mx-auto max-w-6xl px-6 py-10"><h1 className="text-3xl font-extrabold">Flight & tour requests</h1><p className="mt-2 text-slate-600">Requests awaiting an exact travel quotation. Sending a quote emails the client automatically.</p>{error && <p className="mt-6 rounded-xl bg-red-50 p-4 text-sm font-semibold text-red-700">Could not load service requests. Check whether migration 0017 has been applied.</p>}<div className="mt-8 space-y-5">{rows.map((row) => <article key={row.id} className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm"><div className="flex flex-wrap items-start justify-between gap-3"><div><p className="text-xs font-bold uppercase tracking-widest text-blue-600">{row.kind} request · {row.status}</p><h2 className="mt-1 text-xl font-extrabold">{row.full_name}</h2><p className="mt-1 text-sm text-slate-600">{row.email} · {row.phone}</p></div><p className="text-sm text-slate-500">{new Date(row.created_at).toLocaleString()}</p></div><div className="mt-5 grid gap-4 text-sm sm:grid-cols-2 lg:grid-cols-3"><Detail label="Route" value={`${row.origin_city}, ${row.origin_country} → ${row.destination_city || row.destination_country}, ${row.destination_country}`} /><Detail label="Dates" value={`${row.departure_date}${row.return_date ? ` → ${row.return_date}` : ""}`} /><Detail label="Travellers" value={`${row.travellers}${row.kind === "flight" ? ` · Baggage: ${row.baggage_preference ?? "—"}` : ` · ${row.hotel_stars}-star hotel`}`} /><Detail label="Address" value={row.residential_address} />{row.notes && <Detail label="Notes" value={row.notes} />}</div><ServiceRequestActions id={row.id} status={row.status} quote={row.admin_quote} amount={row.quoted_amount_usd} /></article>)}{!error && rows.length === 0 && <p className="rounded-3xl border border-dashed border-slate-300 bg-white py-16 text-center text-slate-500">No flight or tour requests yet.</p>}</div></main>;
}
